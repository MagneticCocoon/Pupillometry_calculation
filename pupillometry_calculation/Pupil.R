...
## 030: Save the result data into a file

#-- Results of all participants
resultPupil <- mutate(resultPupil, IDS = paste(as.numeric(ID),Scenario)) # make IDS column for outlier identification
write.table(resultPupil, file = "Results_Pupil.csv", append = FALSE, sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE)

#-- Individual dataset

filename = substr(filename_n, 17, 22) 
write.table(df_n, file = paste("df_n", filename, ".csv"), append = FALSE, sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE)


## 040: Plot
# 0. Find n of selected participant & produce df_n
participant <- "PA67_S"
n = which(substr(filenames, 17,22) == participant)

sourceLines <- function(fn, startTag, endTag) {
...
sourceLines("1. Pupil.R", "# sourcePartial starts","# sourcePartial ends" )

# 1. Time series of individual (time unit: second)
...
ggplot(df) + 
  ...
  ggtitle(paste("Pupil increase trend for", participant))
}


ggsave(
  paste("Plot_IndividualTrend",participant,".png"),
  ggplot_alternative(),
  width = 10,
  height = 2,
  dpi = 300
...
#-- Window analysis
df %>% filter((Time>1000) & (Time<1500)) %>%
  ggplot( aes(x=Time, y=Pupil_inc, color=IntvName)) + 
  geom_point() + ylim(-8,8) + 
  ggtitle("Pupil increase trend for individual")


# 2. Distribution of all participants
#-- Pupil increase in average
ggplot(resultPupil, aes(Pupil_avrg, colour=Group)) + 
  geom_histogram(aes(y =..density..,fill=Group), alpha = .2, bins=15) + 
  geom_density() + ggtitle("Distribtion of Pupil increase in average")

#-- Pupil increase in blocks
ggplot(resultPupil, aes(Pupil_noPause, colour=Group)) + 
  geom_histogram(aes(y =..density..,fill=Group, alpha = .2, bins=30)) + 
  geom_density() + ggtitle("Distribtion of Pupil increase in blocks")


## 050: PupillometryR: Time series with smoothing

## Smoothing is for looking at trend in a prettified way, averaging sequent entries.
## We don't use this function for this study since we already calculated averaged data above. 
library(PupillometryR)
theme_set(theme_classic(base_size = 12))

df_n <- make_pupillometryr_data(data = df_n,
                                subject = ID,
                                trial = Scenario,
                                time = Time,
                                condition = Group)
df_n <- downsample_time_data(data = df_n,
                             pupil = Pupil_inc,
                             timebin_size = 1,
                             option = 'median')
df_n <- filter_data(data = df_n,
                    pupil = Pupil_inc,
                    filter = 'hanning',
                    degree = 11)
df_n <- interpolate_data(data = df_n,
                         pupil = Pupil_inc,
                         type = 'linear')
plot(df_n, pupil = Pupil_inc, group = 'ID')
